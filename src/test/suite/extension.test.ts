...
type RecordedPersistedSnapshot = {
  workspaceId: string;
  snapshot: PersistedWorkspaceSnapshot;
};

function hasPersistedLayer(snapshot: PersistedWorkspaceSnapshot | undefined, layer: IndexLayer): boolean {
  return snapshot?.metadata.layerState?.availableLayers.includes(layer) ?? false;
}

function isCompletedLayerSnapshot(snapshot: PersistedWorkspaceSnapshot | undefined, layer: IndexLayer): boolean {
  return hasPersistedLayer(snapshot, layer) && snapshot?.metadata.layerState?.activeLayer === undefined;
}

function latestPersistedSnapshot(
  writes: RecordedPersistedSnapshot[],
  predicate: (snapshot: PersistedWorkspaceSnapshot) => boolean
): RecordedPersistedSnapshot | undefined {
  for (let index = writes.length - 1; index >= 0; index -= 1) {
    const candidate = writes[index];
    if (candidate && predicate(candidate.snapshot)) {
      return candidate;
    }
  }
...
suite('extension activation', () => {
  ...
  test('runs symbol indexing with bounded concurrency after the file layer is available', async () => {
    ...
    const registeredCommands = new Map<string, (...args: unknown[]) => unknown>();
    const originalWorkspaceFs = vscode.workspace.fs;
    let activeSymbolCalls = 0;
    let maxActiveSymbolCalls = 0;
    let releaseSymbolCallGate: (() => void) | undefined;
    const symbolCallGate = new Promise<void>((resolve) => {
      releaseSymbolCallGate = resolve;
    });
    ...
    const executePatch = patchProperty(vscode.commands, 'executeCommand', (async (command: string, target?: vscode.Uri) => {
      if (command === 'vscode.executeDocumentSymbolProvider' && target && trackedUris.has(target.toString())) {
        activeSymbolCalls += 1;
        maxActiveSymbolCalls = Math.max(maxActiveSymbolCalls, activeSymbolCalls);
        await symbolCallGate;
        activeSymbolCalls -= 1;
        return [];
      }
      return [];
  ...
  test('restores a file-only snapshot and keeps symbol search blocked until the symbol layer completes', async () => {
    ...
    try {
      ...
      assert.equal(symbolOutcome, 'waiting');
      assert.equal(infoMessages.includes('Building initial symbol index. Please wait a moment.'), true);
      releaseSymbolLayer?.();
  ...
  test('cycle search stays blocked on a file-only snapshot until the symbol layer completes', async () => {
    const workspaceUri = vscode.Uri.file('c:\\workspace');
    const indexedFile = vscode.Uri.parse('file:///workspace/src/app/main.ts');
    const registeredCommands = new Map<string, (...args: unknown[]) => unknown>();
    const infoMessages: string[] = [];
    ...
    try {
      activate({ subscriptions: [] } as unknown as vscode.ExtensionContext);
      await waitFor(() => registeredCommands.has('fastIndexer.cycleSearchMode'), 'cycleSearchMode registration');

      const cycleCommandPromise = Promise.resolve(registeredCommands.get('fastIndexer.cycleSearchMode')?.());
      const cycleOutcome = await Promise.race([
        cycleCommandPromise.then(() => 'resolved'),
        new Promise<'waiting'>((resolve) => setTimeout(() => resolve('waiting'), 20))
      ]);

      assert.equal(cycleOutcome, 'waiting');
      assert.equal(infoMessages.includes('Building initial symbol index. Please wait a moment.'), true);

      releaseSymbolLayer?.();
      await cycleCommandPromise;
    } finally {
      releaseSymbolLayer?.();
      restoreProperty(persistenceWritePatch);
      restoreProperty(persistenceReadPatch);
  ...
  test('restores persisted semantic metadata with indexed symbols and shows semantic detail in goToSymbol', async () => {
    ...
    const persistedSnapshot = {
      metadata: {
        ...
        layerState: {
          availableLayers: ['file', 'text', 'symbol', 'semantic']
        }
      },
      fileIndex: [{
        relativePath: 'src/app/main.ts',
  ...
  test('falls back to the normal build when persisted ignore inputs are incompatible', async () => {
    ...
    let clearedWorkspaceId: string | undefined;
    const persistedWrites: RecordedPersistedSnapshot[] = [];
    const persistedSnapshot = {
      metadata: {
        schemaVersion: 2,
        workspaceId: toExpectedWorkspaceId([workspaceUri]),
    ...
    try {
      ...
      await commandPromise;
      await waitFor(
        () => latestPersistedSnapshot(persistedWrites, (snapshot) => isCompletedLayerSnapshot(snapshot, 'symbol')) !== undefined,
        'rebuilt snapshot after ignore-input invalidation'
      );
  ...
  test('reconciles restored snapshots by pruning deleted files and stale per-phase entries', async () => {
    const workspaceUri = vscode.Uri.file('c:\\workspace');
    const keptFile = vscode.Uri.file('c:\\workspace\\src\\kept.ts');
    const oversizedFile = vscode.Uri.file('c:\\workspace\\src\\oversized.ts');
    const emptySymbolsFile = vscode.Uri.file('c:\\workspace\\src\\emptySymbols.ts');
    const expectedWorkspaceId = toExpectedWorkspaceId([workspaceUri]);
    const persistenceConfigHash = toPersistenceConfigHash({
    ...
    const relativePatch = patchProperty(vscode.workspace, 'asRelativePath', ((pathOrUri: string | vscode.Uri) => {
      ...
      const filename = path.basename(pathOrUri.fsPath);
      return `src/${filename}`;
    }) as typeof vscode.workspace.asRelativePath);
    const workspaceFolderPatch = patchProperty(vscode.workspace, 'getWorkspaceFolder', (() => ({
      uri: workspaceUri,
      index: 0,
      name: 'workspace'
    })) as typeof vscode.workspace.getWorkspaceFolder);
    ...
    const fsPatch = patchProperty(vscode.workspace, 'fs', {
      ...
      stat: async (uri: vscode.Uri) => ({
        ...
        ctime: 0,
        mtime: 0,
        size: uri.fsPath.endsWith('oversized.ts') ? 1024 * 513 : 32
      }),
      readFile: async (uri: vscode.Uri) => Uint8Array.from(Buffer.from(`export const ${path.basename(uri.fsPath, '.ts')} = true;`))
    } as typeof vscode.workspace.fs);
    const executeCommandPatch = patchProperty(vscode.commands, 'executeCommand', (async (command: string, uri?: vscode.Uri) => {
      ...
      if (uri?.fsPath.endsWith('emptySymbols.ts')) {
        return [];
      }

      return [new vscode.DocumentSymbol(
        path.basename(uri?.fsPath ?? 'kept.ts', '.ts'),
        '',
      ...
      (async () => ({
        ...
        fileIndex: [
          ...
          },
          {
            ...
            uri: vscode.Uri.file('c:\\workspace\\src\\removed.ts').toString(),
            basename: 'removed.ts',
            extension: '.ts',
            tokens: ['src', 'removed', 'ts']
          }
        ],
        textIndex: [
          {
            relativePath: 'src/kept.ts',
            uri: keptFile.toString(),
            content: 'export const kept = false;'
          },
          {
            relativePath: 'src/oversized.ts',
            uri: oversizedFile.toString(),
            content: 'stale oversized content'
          },
          {
            relativePath: 'src/removed.ts',
            uri: vscode.Uri.file('c:\\workspace\\src\\removed.ts').toString(),
        ...
        symbolIndex: [
          ...
          {
            relativePath: 'src/emptySymbols.ts',
            symbols: [{
              name: 'OldEmptySymbol',
              kind: vscode.SymbolKind.Function,
              uri: emptySymbolsFile.toString(),
              startLine: 0,
              startColumn: 0,
              approximate: false
            }]
    ...
    try {
      ...
      await waitFor(
        () => latestPersistedSnapshot(persistedWrites, (snapshot) => isCompletedLayerSnapshot(snapshot, 'symbol')) !== undefined,
        'completed reconciled snapshot'
      );

      const reconciledSnapshot = latestPersistedSnapshot(
        persistedWrites,
        (snapshot) => isCompletedLayerSnapshot(snapshot, 'symbol')
      )?.snapshot;
      assert.ok(reconciledSnapshot, 'expected a completed reconciled snapshot');
      assert.deepEqual(reconciledSnapshot.fileIndex.map((entry) => entry.relativePath).sort(), [
        'src/emptySymbols.ts',
        'src/kept.ts',
        'src/oversized.ts'
      ]);
      assert.deepEqual(reconciledSnapshot.textIndex.map((entry) => entry.relativePath).sort(), [
        'src/emptySymbols.ts',
        'src/kept.ts'
      ]);
      assert.deepEqual(
        reconciledSnapshot.symbolIndex
          .filter((entry) => entry.symbols.length > 0)
          .map((entry) => entry.relativePath)
          .sort(),
        ['src/kept.ts', 'src/oversized.ts']
      );
      assert.deepEqual(reconciledSnapshot.semanticIndex?.map((entry) => entry.relativePath), []);
    } finally {
      restoreProperty(persistenceWritePatch);
      restoreProperty(persistenceReadPatch);
      restoreProperty(executeCommandPatch);
  ...
  test('persists the file layer in planner order during startup', async () => {
    const workspaceUri = vscode.Uri.file('c:\\workspace');
    const betaFile = vscode.Uri.file('c:\\workspace\\src\\beta.ts');
    const alphaFile = vscode.Uri.file('c:\\workspace\\src\\alpha.ts');
    const expectedWorkspaceId = toExpectedWorkspaceId([workspaceUri]);
    const persistedWrites: RecordedPersistedSnapshot[] = [];
    const originalWorkspaceFs = vscode.workspace.fs;
    ...
    const relativePatch = patchProperty(vscode.workspace, 'asRelativePath', ((pathOrUri: string | vscode.Uri) => {
      ...
      return `src/${path.basename(pathOrUri.fsPath)}`;
    }) as typeof vscode.workspace.asRelativePath);
    const workspaceFolderPatch = patchProperty(vscode.workspace, 'getWorkspaceFolder', (() => ({
      uri: workspaceUri,
      index: 0,
    ...
    const executeCommandPatch = patchProperty(vscode.commands, 'executeCommand', (async (command: string, uri?: vscode.Uri) => {
      ...
      return [new vscode.DocumentSymbol(
        path.basename(uri?.fsPath ?? 'alpha.ts', '.ts'),
        '',
        vscode.SymbolKind.Function,
        new vscode.Range(0, 0, 0, 10),
        new vscode.Range(0, 0, 0, 10)
    ...
    try {
      ...
      await waitFor(
        () => latestPersistedSnapshot(
          persistedWrites,
          (snapshot) => hasPersistedLayer(snapshot, 'file') && snapshot.metadata.layerState?.activeLayer === 'text'
        ) !== undefined,
        'file-layer checkpoint'
      );

      const fileLayerSnapshot = latestPersistedSnapshot(
        persistedWrites,
        (snapshot) => hasPersistedLayer(snapshot, 'file') && snapshot.metadata.layerState?.activeLayer === 'text'
      )?.snapshot;
      assert.ok(fileLayerSnapshot, 'expected a file-layer checkpoint');
      assert.deepEqual(fileLayerSnapshot.fileIndex.map((entry) => entry.relativePath), [
        'src/alpha.ts',
        'src/beta.ts'
      ]);
      await waitFor(
        () => latestPersistedSnapshot(persistedWrites, (snapshot) => isCompletedLayerSnapshot(snapshot, 'symbol')) !== undefined,
        'completed startup build'
      );
    } finally {
      restoreProperty(persistenceWritePatch);
      restoreProperty(persistenceReadPatch);
  ...
  test('persists a fresh snapshot after a successful initial build', async () => {
    ...
    let persisted: RecordedPersistedSnapshot | undefined;
    ...
    const persistenceWritePatch = patchProperty(
      ...
      (async (workspaceId, snapshot) => {
        ...
        if (isCompletedLayerSnapshot(snapshot, 'symbol')) {
  ...
  test('persists a fresh snapshot after an explicit rebuild completes', async () => {
    ...
    try {
      ...
      await waitFor(
        () => latestPersistedSnapshot(persistedWrites, (snapshot) => isCompletedLayerSnapshot(snapshot, 'symbol')) !== undefined,
        'completed initial snapshot'
      );
      const initialWriteCount = persistedWrites.length;
      ...
      await waitFor(
        () => latestPersistedSnapshot(
          persistedWrites.slice(initialWriteCount),
          (snapshot) => isCompletedLayerSnapshot(snapshot, 'symbol')
        ) !== undefined,
        'completed rebuilt snapshot'
      );

      const rebuiltSnapshot = latestPersistedSnapshot(
        persistedWrites.slice(initialWriteCount),
        (snapshot) => isCompletedLayerSnapshot(snapshot, 'symbol')
      );
      assert.ok(rebuiltSnapshot, 'expected a completed persisted snapshot after rebuild');
      assert.equal(rebuiltSnapshot.workspaceId, expectedWorkspaceId);
      assert.equal(rebuiltSnapshot.snapshot.fileIndex[0]?.relativePath, 'src/app/main.ts');
      assert.equal(rebuiltSnapshot.snapshot.textIndex[0]?.content, 'const value = 1;');
      assert.equal(rebuiltSnapshot.snapshot.symbolIndex[0]?.symbols[0]?.name, 'MainService');
    } finally {
      restoreProperty(persistenceWritePatch);
      restoreProperty(persistenceReadPatch);
      restoreProperty(configListenerPatch);
  ...
  test('applies configured ignore-file rules before persisting the initial index snapshot', async () => {
    ...
    const workspaceUri = vscode.Uri.file(workspaceRoot);
    let resolvePersisted: ((value: RecordedPersistedSnapshot) => void) | undefined;
    const persistedPromise = new Promise<RecordedPersistedSnapshot>((resolve) => {
      resolvePersisted = resolve;
    });
    ...
    const persistenceWritePatch = patchProperty(
      ...
      (async (workspaceId, snapshot) => {
        if (isCompletedLayerSnapshot(snapshot, 'symbol')) {
          resolvePersisted?.({ workspaceId, snapshot });
        }
      }) as typeof PersistenceStore.prototype.writeWorkspaceSnapshot
    );
  ...
  test('keeps missing configured ignore files non-fatal and surfaces diagnostic output', async () => {
    ...
    try {
      ...
      await waitFor(() => persistedSnapshot !== undefined, 'persisted snapshot with missing ignore file');
      assert.ok(
  ...
  test('persists ignore configuration and loaded ignore inputs in snapshot metadata', async () => {
    ...
    const persistenceWritePatch = patchProperty(
      ...
      (async (_workspaceId, snapshot) => {
        if (isCompletedLayerSnapshot(snapshot, 'symbol')) {
          persisted = {
            workspaceId: workspaceUri.toString(),
            snapshot
          };
        }
      }) as typeof PersistenceStore.prototype.writeWorkspaceSnapshot
  ...
  test('ignores watcher updates from excluded heavy paths', async () => {
    ...
    const persistenceWritePatch = patchProperty(
      ...
      'writeWorkspaceSnapshot',
      (async (workspaceId, snapshot) => {
        if (workspaceId === expectedWorkspaceId && isCompletedLayerSnapshot(snapshot, 'symbol')) {
          persistedSnapshot = snapshot;
        }
      }) as typeof PersistenceStore.prototype.writeWorkspaceSnapshot
    );
    ...
    try {
      ...
      assert.ok(goToFileCommand, 'goToFile command should be registered');
      await Promise.resolve(goToFileCommand?.());
      await waitFor(() => persistedSnapshot !== undefined, 'initial build completion');
  ...
  test('refreshes ignore-file watcher behavior after a configured ignore file changes', async function () {
    ...
    const originalMarkReady = IndexCoordinator.prototype.markReady;
    ...
    const markReadyPatch = patchProperty(
      IndexCoordinator.prototype,
      'markReady',
      (function (this: IndexCoordinator) {
        readyMarks += 1;
        return originalMarkReady.call(this);
      }) as typeof IndexCoordinator.prototype.markReady
    );
    const persistenceReadPatch = patchProperty(
      PersistenceStore.prototype,
      'readWorkspaceSnapshot',
    ...
    try {
      ...
      await waitFor(() => readyMarks >= 1, 'initial build completion');
      staleMarks = 0;
      ...
      restoreProperty(persistenceWritePatch);
      restoreProperty(persistenceReadPatch);
      restoreProperty(markReadyPatch);
  ...
  test('shows indexing progress in the status bar during a large workspace build', async function () {
    ...
    const persistenceWritePatch = patchProperty(
      ...
      (async (workspaceId, snapshot) => {
        if (
          workspaceId === expectedWorkspaceId
          && isCompletedLayerSnapshot(snapshot, 'symbol')
          && snapshot.fileIndex.length === files.length
        ) {
          persistedSnapshot = snapshot;
        }
      }) as typeof PersistenceStore.prototype.writeWorkspaceSnapshot
    ...
    try {
      ...
      assert.ok(goToFileCommand, 'goToFile command should be registered');
      await Promise.resolve(goToFileCommand?.());
      await waitFor(() => persistedSnapshot !== undefined, 'workspace build completion');
    } finally {
      restoreProperty(persistenceWritePatch);
      restoreProperty(persistenceReadPatch);
      restoreProperty(outputPatch);
  ...
  test('semantic enrichment does not block initial snapshot persistence when providers are slow', async () => {
    ...
    try {
      ...
      await waitFor(
        () => semanticProviderStarted && persistedSnapshot !== undefined,
        'completed snapshot persistence after semantic enrichment starts',
        2000
      );

      assert.ok(semanticProviderStarted, 'semantic enrichment should have started');
      assert.ok(hasPersistedLayer(persistedSnapshot, 'symbol'), 'snapshot should be persisted even when semantic providers are slow');
    } finally {
      restoreProperty(persistenceWritePatch);
      restoreProperty(persistenceReadPatch);
      restoreProperty(configListenerPatch);
  ...
  test('times out slow document symbol providers during initial indexing and still persists the snapshot', async () => {
    ...
    try {
      ...
      await waitFor(
        () => persistedSnapshot !== undefined && outputLines.some((line) => line.includes('Timed out reading document symbols for src/app/main.ts')),
        'initial snapshot persistence after symbol timeout',
        2000
      );
    } finally {
      restoreProperty(persistenceWritePatch);
    ...
    assert.ok(hasPersistedLayer(persistedSnapshot, 'symbol'), 'snapshot should be persisted even when symbol providers hang');
  });
});
